...
describe("Endpoint - /orders?userId=:id", () => {
  test("should return all orders for selected user on GET request", async () => {
    ...
    const selectedId = JSON.parse(orders)[0].id;
...
describe("Endpoint - /order/:id", () => {
  test("should return Order by id", async () => {
    const fetchedData = await fetch(
      `${URL}/order/abdf9c2b-32ba-42d6-806c-c99da988e9ca`
    ).then((res) => res.json());

    expect(fetchedData.id).toEqual("abdf9c2b-32ba-42d6-806c-c99da988e9ca");
  });
